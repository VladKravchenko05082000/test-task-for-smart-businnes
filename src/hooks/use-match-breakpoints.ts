...
const mediaQueries: MediaQueries = (() => {
  ...
  return (
    Object.keys(breakpointMap) as Array<keyof typeof breakpointMap>
  ).reduce((accum, size, index) => {
    ...
    if (sizeToSet === Object.keys(breakpointMap)[0]) {
      return {
        ...accum,
        [sizeToSet]: `(min-width: ${0}px) and (max-width: ${breakpoint}px)`,
      };
    }
...
const getKey = (size: keyof MediaQueries) =>
  `is${size.charAt(0).toUpperCase()}${size.slice(1)}`;

const getState = () => {
  const s = (Object.keys(mediaQueries) as Array<keyof MediaQueries>).reduce(
    (accum, size) => {
...
const useMatchBreakpoints = () => {
  ...
  React.useEffect(() => {
    ...
    const handlers = (
      Object.keys(mediaQueries) as Array<keyof MediaQueries>
    ).map((size) => {
